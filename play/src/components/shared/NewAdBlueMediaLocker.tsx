...
export function NewAdBlueMediaLocker({
  ...
  const checkAdBlocker = useCallback((): boolean => {
    try {
      ...
      const isBlocking = testDiv.offsetHeight === 0 || testDiv.offsetParent === null;
  ...
  useEffect(() => {
    ...
    const originalCallback = window.nUtRu_QzT_cplrFc;
  ...
  const handleRetry = () => {
    ...
    if (!checkAdBlocker() && typeof window._iH === 'function') {
      try {
        window._iH();
        setIsLoading(false);
      } catch (error) {

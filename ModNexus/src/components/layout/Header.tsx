...
import { FileDown, Menu, X, User, Gamepad2, Clock, Star, ChevronRight, Grid, Heart } from 'lucide-react';
...
export function Header() {
  ...
  const { favoriteGames } = useUserPreferences();
  ...
  useEffect(() => {
    if (profileOpen) {
      ...
      const games = GAME_RESOURCES.filter(game =>
        localStorage.getItem(`unlocked_${game.id}`) === 'true'
      );
  ...
  const userLevel = Math.floor(downloadedGames.length / 2) + Math.floor(userReviews.length / 3) + 1;
    ...
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      ...
      <div className="container-custom flex items-center justify-between h-16">
        ...
        <Link to="/" className="flex items-center">
          <img src="/assets/logo/modnexus-logo.svg" alt="ModNexus" className="h-10 logo-glow" />
        </Link>
        ...
        <div className="flex items-center space-x-2">
          ...
          <Link to="/categories" className="hidden md:block">
            ...
            >
              <Heart className={`h-4 w-4 mr-2 ${favoriteGames.length > 0 ? "fill-red-500 text-red-500" : ""}`} />
              Favorites
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            <SheetTrigger asChild>
              <Button variant="outline" size="icon" className="md:hidden border-[#00f7ff]/30 hover:bg-[#00f7ff]/10">
                <Menu className="h-5 w-5" />
              </Button>
            </SheetTrigger>
            <SheetContent side="right" className="border-l-[#00f7ff]/20">
              <div className="flex flex-col h-full">
                <div className="flex items-center justify-between mb-6">
                  <img src="/assets/logo/modnexus-logo.svg" alt="ModNexus" className="h-8 logo-glow" />
                ...
                <nav className="flex flex-col space-y-4">
                  ...
                  <Link to="/#games" className="mobile-nav-link" onClick={() => setMobileMenuOpen(false)}>
                    Games
                  </Link>
                ...
                <div className="mt-auto pt-6 border-t border-[#00f7ff]/10 text-center text-xs text-muted-foreground">
                  © 2025 ModNexus. All rights reserved.
                </div>
              </div>
            </SheetContent>
          </Sheet>
          ...
          <div className="mt-4 space-y-8">
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                <div className="bg-background/50 rounded p-2 text-center">
                  <div className="flex justify-center mb-1">
                    <Gamepad2 className="h-4 w-4 text-[#00f7ff]" />
                  </div>
                  <div className="text-xl font-semibold">{totalDownloads}</div>
                  <div className="text-xs text-muted-foreground">Games Downloaded</div>
                </div>
                <div className="bg-background/50 rounded p-2 text-center">
                  <div className="flex justify-center mb-1">
                  ...
                  <div className="text-xs text-muted-foreground">Reviews Posted</div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-lg font-semibold mb-3 flex items-center">
                <Gamepad2 className="h-5 w-5 text-[#00f7ff] mr-2" />
                My Games
              </h3>
              ...
              {downloadedGames.length > 0 ? (
                ...
                <div className="text-center py-4 border border-dashed border-[#00f7ff]/20 rounded-lg">
                  ...
                  <Link to="/#games">
                    <Button variant="outline" size="sm" className="mt-2 border-[#00f7ff]/30 hover:bg-[#00f7ff]/10">
                      Browse Games
                    </Button>
                  </Link>
                </div>
                  ...
                  {userReviews.slice(0, 3).map(review => (
                    <Link to={`/games/${review.gameId}`} key={review.id}>
                      <div className="flex items-start gap-3 p-2 rounded-lg hover:bg-card/80 transition-colors">
                        ...
                        <div className="flex-1">
                          <p className="text-sm">
                            You reviewed <span className="font-medium">{review.gameTitle}</span>
                          </p>
